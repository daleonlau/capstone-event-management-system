...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // No status checking - if insights exist, show them!
        $aiInsightsList = [];
        
        try {
            // Get all AI analyses for evaluations belonging to this organization's events
            $analyses = AIAnalysis::whereHas('evaluation.event', function($q) {
                    $q->where('user_id', $this->organizationId);
                })
                ->whereNull('event_date') // Overall insights only (not per-date)
            ...
            foreach ($analyses as $analysis) {
                ...
                // Calculate response rate if not stored
                $responseRate = $analysis->response_rate ? ($analysis->response_rate * 100) : 0;
            ...
            // Log to debug
            \Log::info('Adviser Dashboard AI Insights Count: ' . count($aiInsightsList));
