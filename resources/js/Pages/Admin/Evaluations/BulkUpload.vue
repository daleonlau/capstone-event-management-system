<template>
  <div class="bg-white rounded-2xl shadow-lg p-6">
    ...
    <!-- Event Dates Info Card -->
    <div v-if="eventDates && eventDates.length > 0" class="bg-blue-50 rounded-xl p-4 mb-4 border border-blue-200">
      ...
      <div class="flex flex-wrap gap-2 mb-2">
        <span v-for="(date, idx) in eventDates" :key="date" 
              class="px-3 py-1 bg-white rounded-full text-sm border border-blue-200">
          Day {{ idx + 1 }}: {{ formatDate(date) }}
        </span>
      </div>
      <p class="text-xs text-blue-600">
        <strong>Important:</strong> The <code>event_date</code> column in your CSV must match one of these dates exactly.
        Each student/guest can submit once per date.
      </p>
    </div>

    <div class="text-sm text-gray-600 mb-4">
      <p>Upload a CSV file with responses. The template includes all necessary columns.</p>
      <ul class="list-disc list-inside mt-2 text-xs text-gray-500 space-y-1">
        <li>First row: Column headers (do not modify header names)</li>
        <li>Second row: Sample data (you can replace or delete)</li>
        <li><strong>event_date</strong> is required - use format YYYY-MM-DD (e.g., 2026-03-26)</li>
        <li>Ratings must be numbers 1-5 (1=Very Dissatisfied, 5=Very Satisfied)</li>
        <li>For students: use Student ID; For guests: use registered name (not Guest ID)</li>
      </ul>
    </div>
