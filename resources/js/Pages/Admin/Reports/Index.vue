    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
          ...
          <p class="text-gray-500 mt-1">Generate and send comprehensive evaluation reports with AI-powered insights</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div class="bg-white rounded-2xl shadow-lg p-6">
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="text-sm text-gray-500">AI Insights Available</p>
                <p class="text-2xl font-bold text-amber-600">{{ insightsCount }}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <table class="w-full">
              <thead class="bg-emerald-500">
                <tr>
                  ...
                  <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Event Dates</th>
                  ...
                  <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">AI Insights</th>
                ...
                <tr v-for="evaluation in evaluations" :key="evaluation.id" class="hover:bg-gray-50 transition">
                  <td class="px-6 py-4">
                    <div>
                      ...
                      <p class="text-xs text-gray-500">{{ formatDate(evaluation.event_date_start) }} - {{ formatDate(evaluation.event_date_end) }}</p>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <div class="flex flex-wrap gap-1">
                      <span v-for="date in evaluation.event_dates" :key="date.date" 
                            class="px-2 py-1 text-xs rounded-full"
                            :class="date.response_count > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
                        {{ date.formatted_date }}
                        <span class="ml-1">({{ date.response_count }})</span>
                      </span>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex flex-col gap-1">
                      <div v-if="evaluation.has_ai_insights" class="flex items-center gap-1">
                      ...
                      <div v-if="evaluation.date_insights_count > 0" class="text-xs text-blue-600">
                        {{ evaluation.date_insights_count }} date-specific insights
                      </div>
                      <div v-if="!evaluation.has_ai_insights && evaluation.date_insights_count === 0" class="text-xs text-gray-400">
...
const insightsCount = computed(() => {
  return props.evaluations.filter(e => e.has_ai_insights || e.date_insights_count > 0).length;
});
...
async function generateReport(evaluation) {
  ...
  showToast('Generating comprehensive report with AI insights...', 'info');
  
  try {
    const response = await axios.post(`/admin/reports/${evaluation.id}/generate`);
    
    if (response.data.success) {
      showToast('✅ Report generated successfully with AI insights!', 'success');
...
async function regenerateReport(evaluation) {
  ...
  if (!confirm('Are you sure you want to regenerate this report? This will update the report with the latest data and AI insights.')) return;
  
  regenerating.value[evaluation.id] = true;
  showToast('Regenerating report with latest data and AI insights...', 'info');
  
  try {
    const response = await axios.post(`/admin/reports/${evaluation.id}/regenerate`);
    
    if (response.data.success) {
      showToast('✅ Report regenerated successfully with latest AI insights!', 'success');
      setTimeout(() => router.reload(), 1500);
    } else {
